...
export function useCaptureSettings() {
  ...
  const mutation = useMutation({
    ...
    onSettled: (data, _err, patch) => {
      ...
      // /capture/readiness resolves stt_model live on each call, but its
      // cached response keeps serving the previous model's state until the
      // next 5 s poll. Invalidate on model swaps so the readiness checklist
      // re-checks immediately.
      if (patch.stt_model !== undefined) {

 ...
 *
 * The pill surfaces one cycle: user dictation, driven by ``dictate:start`` /
 * ``dictate:stop`` from the Rust hotkey monitor. (The upstream agent-speech
...
export function DictateWindow() {
  ...
  const effectiveElapsed = session.pillElapsedMs;
